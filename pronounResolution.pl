%process and append predicate written by Aravind Venkateswaran z5208102
...
process(event(X,[H|T]), Ref1, Ref2):-   
    process([H|T],Ref1,Ref2),
    assert(history(event(X,[H|T]))).

process([H|T],Ref1,Ref2):-
    process(H,[],Ref3),
    process(T,[],Ref4),
	append(Ref3,Ref4,Ref2).

process(actor(X),Ref1,Ref2):-
    process(X,Ref1,Ref3),
	append(Ref3,Ref1,Ref2).

process(thing(X,_),Ref1,Ref2):-
   thing(X, Y), 
   assert(history(thing(X, Y))),
   append([],Ref1,Ref2).

process(object(X),Ref1,Ref2):-
    process(X,Ref1,Ref3),
	append(Ref3,Ref1,Ref2).


process(possessive(X,Y),Ref1,Ref3):-
	possessive(X, Props1),
...
process(set(X,Y),Ref1,Ref2) :-
    process(X,Ref1,Ref3),
	process(Y,Ref3,Ref4),
	append(Ref4,Ref1,Ref2).
